					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$tenant = $conn->query("SELECT t.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.house_no,h.price, c.name as rooms FROM tenants t inner join houses h on h.id = t.house_id INNER JOIN categories c ON h.category_id = c.id  where t.status = 1 order by h.house_no desc ");
								while($row=$tenant->fetch_assoc()):
									...
									$room = $conn->query("SELECT c.name AS room, h.house_no FROM categories as c INNER JOIN ON houses AS h WHERE c.id = h.house_no AND tenant_id =".$row['id']);
