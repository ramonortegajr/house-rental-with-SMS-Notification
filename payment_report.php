						...
						<div class="row">
							<table class="table table-bordered">
								...
								<tbody>
									...
									$i = 1;
									$tamount = 0;
									$payments  = $conn->query("SELECT p.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.house_no, c.name as room FROM payments p inner join tenants t on t.id = p.tenant_id inner join houses h on h.id = t.house_id inner join categories c on h.category_id = c.id where date_format(p.date_created,'%Y-%m') = '$month_of' order by unix_timestamp(date_created)  asc");
									...
									<tr>
										...
										<td><?php echo ucwords($row['room']) ?></td>
